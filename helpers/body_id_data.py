...
def create_test_entity(booker_api):
    """Создает тестовую сущность и возвращает ее id."""
    data = TestDictForRequests().return_dict_other()
    ent = booker_api.post(data=json.dumps(data))
    id_to_do_request = ent.json()['bookingid']
    return id_to_do_request


@allure.step("Удаляем тестовую сущность")
def delete_test_entity(booker_api, param):
